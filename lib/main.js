...
exports.main = function() {
    ...
    var checkRemove = function(aTab) {
        if (aTab.__responsiveUI || aTab.responsiveUI) {
            // Remove my things
            (aTab.__responsiveUI || aTab.responsiveUI).stack.firstElementChild.removeAttribute("style");
            if ((aTab.__responsiveUI || aTab.responsiveUI).menulist2) {
    ...
    var checkAdd = function(aTab) {
        if (aTab.__responsiveUI || aTab.responsiveUI) {
            // Add my things
            let rui = aTab.__responsiveUI || aTab.responsiveUI;
    ...
    ResponsiveUIManager.handleGcliCommand = function (aWindow, aTab, aCommand, aArgs) {
        checkRemove(aTab);
        oldHandleGcliCommand(aWindow, aTab, aCommand, aArgs);
        checkAdd(aTab);
    }
